...
function Post({ post }: Props) {
  ...
  return (
    <main>
      ...
      <article className="max-w-3xl mx-auto p-5">
        ...
        <div className="mt-10">
          <PortableText
            ...
            serializers={{
              h1: (props: any) => (
                <h1 className="text-2x1 font-bold my-5"  {...props} />),
              h2: (props: any) => (
                <h1 className="text-xl font-bold my-5" {...props} />),
              li: ({ children }: any) => (
                <li className="m-4 list-disc"> {children} </li>),
              // img: ({ src }: any) => (
              //   <img src={src} alt="" className="mt-10 rounded-full" />),
              link: ({ href, children }: any) => (
                <a href={href} className="fl text-blue-500 hover:underline">
                  {children}</a>
              ),
            }}
          />
      ...
      {submitted ? (
        <div className="flex flex-col p-10 my-10 bg-yellow-600 text-white max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col p-5 max-w-2xl mx-auto mb-10">
          ...
          <label className="block mb-5">
            ...
            <input {...register("email", { required: true })} className="shadow border rounded py-2 px-3 form-input block w-full ring-yellow-500 outline-none focus:ring" placeholder="John Appleseed" type="email" /> </label>
          <label className="block mb-5">
          ...
          <input type="submit" className="shadow bg-yellow-600 hover:bg-yellow-500 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded cursor-pointer" />
        </form>
